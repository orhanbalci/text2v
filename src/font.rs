...
impl OutlineBuilder for Outliner<'_> {
    ...
    fn quad_to(&mut self, x1: f32, y1: f32, x: f32, y: f32) {
        ...
        self.path.push(PathEl::QuadTo(
            Point::new(x1 as f64 * self.scale as f64, y1 as f64 * self.scale as f64),
            Point::new(x as f64 * self.scale as f64, y as f64 * self.scale as f64),
        ));
    }
    ...
    fn curve_to(&mut self, x1: f32, y1: f32, x2: f32, y2: f32, x: f32, y: f32) {
        ...
        self.path.push(PathEl::CurveTo(
            Point::new(x1 as f64 * self.scale as f64, y1 as f64 * self.scale as f64),
            Point::new(x2 as f64 * self.scale as f64, y2 as f64 * self.scale as f64),
            Point::new(x as f64 * self.scale as f64, y as f64 * self.scale as f64),
        ));
    }
...
impl<'a> StyledFont<'a> {
    fn path(&self, index: usize, path: &mut BezPath, offset: &mut (i32, i32)) {
        let GlyphInfo {
            glyph_id,
            cluster: _,
            ..
...
impl Iterator for TextPathIterator<'_, '_> {
    ...
    fn next(&mut self) -> Option<PathEl> {
        ...
        if self.path_i != self.fontc.paths.iter().count() {
            let path_op = self.fontc.paths.iter().nth(self.path_i);
            self.path_i += 1;
            return path_op;
        }
        // Because no path ops were left, clear buffer for reuse.
        self.fontc.paths = BezPath::new();
        self.path_i = 0;
